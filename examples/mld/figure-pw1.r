...
## Clean up data...
names(small) <- c("0.0","0.01","0.1","1.0","3.0","10.0")
names(medium) <- c("0.0","0.01","0.1","1.0","3.0","10.0")
names(large) <- c("0.0","0.01","0.1","1.0","3.0","10.0")
...
matplot(seq(500,5000),small,type='l',lwd=1,xlab="Time Step (t)",
        ...
        xaxt="n")
## legend("bottomright",names(small),lty=seq(1,ncol(small)),lwd=1,
##        col=seq(1,ncol(small)),bg="white",title=expression(sigma[theta*phi]))
...
##        col=seq(1,ncol(medium)),bg="white",title=expression(sigma[theta*phi]))
...
legend("topright",names(large),lty=seq(1,ncol(large)),lwd=1,cex=0.75,
       col=seq(1,ncol(large)),bg="white",title=expression(sigma[theta*phi]))
