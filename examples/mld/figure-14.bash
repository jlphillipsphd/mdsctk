...
echo
echo "Figure 14 - NUP116 dimensionality estimation results"
echo

echo "Unsmoothed results..."
echo "Converting protein frames into phi-psi angle space..."
bb_xtc_to_phipsi \
    -x traj/proteins/nup116.01.bb.xtc \
    -o nup116-unsmoothed-phipsi.dat
    
echo "Converting phi-psi angles to Euclidean sin-cos space.."
angles_to_sincos \
    -i nup116-unsmoothed-phipsi.dat \
...
${MDSCTK_HOME}/mld_estimate.r -k ${KNN} -w ${WINDOW} \
    -d nup116-unsmoothed-distances.dat \
    -o nup116-unsmoothed-estimates.dat
...
${MDSCTK_HOME}/mld_estimate.r -k ${KNN} -w ${WINDOW} \
    -d nup116-smoothed-distances.dat \
    -o nup116-smoothed-estimates.dat

echo "Computing RMSD from main structure (folded-NFP or average-IDP)..."
${MDSCTK_HOME}/rms_test \
    -p traj/proteins/nup116.ca.avg.gro \
    -x traj/proteins/nup116.01.ca.xtc \
    -o nup116-rmsd.dat
